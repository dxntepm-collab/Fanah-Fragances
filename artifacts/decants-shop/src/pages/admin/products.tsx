    ...
    <AdminLayout>
      <div className="p-10">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-2xl font-serif text-[#1DD9D4]">Perfumes</h1>
          <button
            onClick={handleNew}
              ...
              {products.map((p) => (
                <tr key={p.id} className="border-b border-white/5 hover:bg-white/5">
                  ...
                  <td className="text-neutral-200">
                    <Link href={`/admin/productos/${p.id}`} className="hover:text-[#1DD9D4]">
                      {p.name}
                    </Link>
                  ...
                  <td className="text-center">{p.isFeatured && <Star className="w-4 h-4 text-[#1DD9D4] inline" />}</td>
                  <td className="text-center">{p.isNew && <Sparkles className="w-4 h-4 text-[#1DD9D4] inline" />}</td>
                  <td className="text-right whitespace-nowrap pr-2">
                    <Link href={`/admin/productos/${p.id}`} className="text-[#1DD9D4] mr-3" title="Editar">
